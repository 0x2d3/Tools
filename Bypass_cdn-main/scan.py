...
def banner():
    print('''            _______ ______  _____  ______   _______    _____ _  _   _____ 
          |__   __|  ____|  __ \|  ____| |  ____|  / ____| | | |  __ \ 
             | |   | |__  | |__) | |__    | |__    | (___  | | | | |__) |
             | |   |  __| |  _  /|  __|   |  __|   \___ \ | | | |  _  / 
             | |   | |____| | \ \| |____  | |____  ____) || | | | | \ \ 
             |_|   |______|_|  \_\______| |______||_____/ |_| |_|_|  \_\
                                                               
               _    _    ______  _   _    ____   ___   __   __ 
              | |  | |  |  ____|| \ | |  |  _ \ / _ \  \ \ / / 
              | |__| |  | |__   |  \| |  | |_) | | | |  \ V /  
              |  __  |  |  __|  | . ` |  |  _ <| | | |  > <   
              | |  | |  | |____ | |\  |  | |_) | |_| | / . \ 
              |_|  |_|  |______||_| \_|  |____/ \___/ /_/ \_\
                   
                         BY 0x2d3 . 0x2d3's WeChat ID : ANONYMOUS-CN23.
                         提醒:
                         {......不要用代理,不然运行不了......}
                         {......oneforall放根目录就行,记得安装一下oneforall的依赖.......}
''')
...
class bypass_cdn(object):
    ...
    def check_phpinfo(self):
        ...
        for phpinfo in payloads:
            # test_url = self.scheme + "://" + self.target.rstrip('/') + "/" +
            # phpinfo
                    ...
                    for i in org_links:
                        if "SERVER_ADDR" in str(i):
                            ...
                            if not ret.startswith(
                                    '192.168') and not ret.startswith('10.'):
                                self.result.add(ret)
            except Exception as e:
                # print(e)
                pass
    ...
    # def write_to_file(subdomain, ip, filename)
    ...
    def subscan(self):
        ...
        cmdline = "python subDomainsBrute.py {}".format(target)
        print(f"[+] 执行 SubDomainBrute : {cmdline}")
        os.system(cmdline)
        # print(cmdline)
        os.chdir(self.Root_Path)
        sub_filename = "./subDomainsBrute/{}.txt".format(target)
        if not os.path.exists(sub_filename):
            print(f"[-] file {sub_filename} not exist,please check it.")
            return
        ...
        with open(sub_filename, "r") as f:
            ...
            for x in content.strip().split('\n'):
                try:
                    ...
                    pass
        print("[+] 开始执行 OneForAll ...")

        oneforall_dir = "./oneforall"
        os.chdir(oneforall_dir)

        cmdline = f"python oneforall.py --target {target} run"
        print(f"[+] 执行 OneForAll : {cmdline}")
        try:
            result = subprocess.run(
                cmdline,
                shell=True,
                capture_output=True,
                # stdout=sys.stdout, 如果想显示oneforall的过程去注释 再将capture_output注释
                # stderr=sys.stderr,
                text=True,
                timeout=60)
            # result = subprocess.run(cmdline, shell=True, capture_output=True, text=True, timeout=60) 
            if result.returncode == 0:
                print("[+] OneforAll 执行成功")
            else:
                print("[-] OneForAll 执行失败 Please check it...")

        except subprocess.TimeoutExpired:
            print("[-] 执行 OneForAll 超时")
            time.sleep(5)

        os.chdir(self.Root_Path)

        target_no_www = target
        if target_no_www.startswith("www."):
            target_no_www = target_no_www[4:]
        

        sub_filename = os.path.join(oneforall_dir, "results", f"{target_no_www}.csv")

        if not os.path.exists(sub_filename):
            print("[-] OneForAll 扫描结果{sub_filename} 不存在, Please check it")
            return

        ips_subdomains = []
        with open(sub_filename, "r", newline='') as f:
            ...
            for row in reader:
                try:
                    ...
                    ips_subdomains.append((subdomain, ip))
                except KeyError:
                    print(f"[-] 无法解析:{row}")
                    continue
        ...
        sub_filename_tmp = "./subDomainsBrute/{}.txt".format(target)

        with open (sub_filename_tmp, "r+", encoding='utf-8', errors='ignore') as f:
            content = f.read()
            
            processed_lines = set()
            for x in content.strip().split('\n'):
                try:
                    subdomain, ip = x.split('\t')
                    if subdomain is None or ip is None or ip == 'None' or subdomain == ip:
                        continue
                    processed_lines.add((subdomain.strip(), ip.strip()))
                
                except ValueError:
                    continue
                
        with open(sub_filename_tmp, "a", encoding='utf-8', errors='ignore') as f:
            for subdomain, ip in processed_lines:
                f.write(f"{subdomain}\t{ip}\n")
                
        with open(sub_filename_tmp, "a", encoding='utf-8', errors='ignore') as f:
            for subdomain, ip in ips_subdomains:
                if subdomain is None or ip is None or ip == 'None' or subdomain == ip:
                    continue
                if (subdomain.strip(), ip.strip()) not in processed_lines:
                    f.write(f"{subdomain}\t{ip}\n")
                    processed_lines.add((subdomain.strip(), ip.strip()))
    ...
    def run(self):
        ...
        while len(self.cidr_set) != 0:
            ...
            # print(type(cidr))
            for ip in cidr:
                target = str(self.scheme) + "://" + \
                    str(ip) + ":" + str(self.port)
